/*
 * (C) Copyright IBM Corp. 2026.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with
 * the License. You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on
 * an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the
 * specific language governing permissions and limitations under the License.
 */

package com.ibm.cloud.watsonxdata.watsonx_data.v3.model;

import com.ibm.cloud.sdk.core.service.model.GenericModel;

/**
 * The listCatalogs options.
 */
public class ListCatalogsOptions extends GenericModel {

  protected String authInstanceId;
  protected String secret;
  protected Boolean defaultCatalogs;
  protected String view;

  /**
   * Builder.
   */
  public static class Builder {
    private String authInstanceId;
    private String secret;
    private Boolean defaultCatalogs;
    private String view;

    /**
     * Instantiates a new Builder from an existing ListCatalogsOptions instance.
     *
     * @param listCatalogsOptions the instance to initialize the Builder with
     */
    private Builder(ListCatalogsOptions listCatalogsOptions) {
      this.authInstanceId = listCatalogsOptions.authInstanceId;
      this.secret = listCatalogsOptions.secret;
      this.defaultCatalogs = listCatalogsOptions.defaultCatalogs;
      this.view = listCatalogsOptions.view;
    }

    /**
     * Instantiates a new builder.
     */
    public Builder() {
    }

    /**
     * Builds a ListCatalogsOptions.
     *
     * @return the new ListCatalogsOptions instance
     */
    public ListCatalogsOptions build() {
      return new ListCatalogsOptions(this);
    }

    /**
     * Set the authInstanceId.
     *
     * @param authInstanceId the authInstanceId
     * @return the ListCatalogsOptions builder
     */
    public Builder authInstanceId(String authInstanceId) {
      this.authInstanceId = authInstanceId;
      return this;
    }

    /**
     * Set the secret.
     *
     * @param secret the secret
     * @return the ListCatalogsOptions builder
     */
    public Builder secret(String secret) {
      this.secret = secret;
      return this;
    }

    /**
     * Set the defaultCatalogs.
     *
     * @param defaultCatalogs the defaultCatalogs
     * @return the ListCatalogsOptions builder
     */
    public Builder defaultCatalogs(Boolean defaultCatalogs) {
      this.defaultCatalogs = defaultCatalogs;
      return this;
    }

    /**
     * Set the view.
     *
     * @param view the view
     * @return the ListCatalogsOptions builder
     */
    public Builder view(String view) {
      this.view = view;
      return this;
    }
  }

  protected ListCatalogsOptions() { }

  protected ListCatalogsOptions(Builder builder) {
    authInstanceId = builder.authInstanceId;
    secret = builder.secret;
    defaultCatalogs = builder.defaultCatalogs;
    view = builder.view;
  }

  /**
   * New builder.
   *
   * @return a ListCatalogsOptions builder
   */
  public Builder newBuilder() {
    return new Builder(this);
  }

  /**
   * Gets the authInstanceId.
   *
   * watsonx.data instance ID.
   *
   * @return the authInstanceId
   */
  public String authInstanceId() {
    return authInstanceId;
  }

  /**
   * Gets the secret.
   *
   * API Authentication service token.
   *
   * @return the secret
   */
  public String secret() {
    return secret;
  }

  /**
   * Gets the defaultCatalogs.
   *
   * Default catalogs flag for cornerstone feature.
   *
   * @return the defaultCatalogs
   */
  public Boolean defaultCatalogs() {
    return defaultCatalogs;
  }

  /**
   * Gets the view.
   *
   * Internal view parameter for cornerstone feature.
   *
   * @return the view
   */
  public String view() {
    return view;
  }
}

